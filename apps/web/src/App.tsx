...
function RuntimeWorkspace({ route, onHome, onExitContext }: {
  ...
  return <div className={`app-shell ${immersivePlay ? "immersive-play" : ""}`}>
    ...
    {route.view === "director" && live.connectionError ? <div className="error-banner"><span>{live.connectionError}</span><button type="button" onClick={live.reconnect}>Reconnect</button></div> : null}
    {route.view === "director" && live.commandError ? <div className="error-banner command-error-banner" role="alert"><span>{live.commandError}</span><button type="button" onClick={live.dismissCommandError}>Dismiss</button></div> : null}
    {route.view === "director" ? (
